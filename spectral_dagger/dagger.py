...
def dagger(
        ...
        initial_policy=None):
    """
    Fully observable DAgger. Operates on an MDP and learns policies
    that operate on states of that MDP.
    """
    ...
    for i in range(n_iterations):
        ...
        for j in range(n_samples_per_iter):
            ...
            for t in range(horizon):
                ...
                next_state, _ = mdp.execute_action(action)
...
def po_dagger(
        ...
        initial_policy=None):
    """
    Partially Observable DAgger. Operates on a POMDP and learns policies
    that operate on action-observation histories generated by the POMDP.
    """

    print "Running PO-DAgger..."

    if initial_policy is None:
        initial_policy = POUniformRandomPolicy(pomdp)
...
class StateClassifier(LearningAlgorithm):
    ...
    an expert. Uses a classifier to try to reproduce the mapping.
    ...
    def fit(self, mdp, states, actions):
        ...
        state_vectors = np.array([state.as_vector() for state in states])
        ...
        classifier.fit(state_vectors, str_actions)
...
class BeliefStateClassifier(LearningAlgorithm):
    ...
    def fit(self, pomdp, trajectories, actions):
        ...
        classifier.fit(belief_states, str_actions)

        def f(belief_state):
            action_string = classifier.predict(belief_state)[0]
            return action_lookup[action_string]
